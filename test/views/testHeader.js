...
describe("<HeaderWeeks />", () => {
  it("consists of proper elements", () => {
    ...
    assert(
      wrapper.children().every(Table.HeaderCell),
      "top node contains nodes: <Table.HeaderCell />"
    );
    assert.equal(
      wrapper.children().getElements().length,
      7,
      "top node contains 7 nodes"
    );
  });

  it("has proper localization", () => {
    const wrapper = shallow(<HeaderWeeks localization="ru" />);
    const weekArray = wrapper
      .children()
      .getElements()
      .map(element => moment(element.key, "ddd dddd", "ru").isValid());
  ...
  it("has proper styling", () => {
    ...
    wrapper.children().forEach(child => {
      ...
      assert.equal(
        _.keys(style).length,
        2,
        "each child of top node has prop style: { border, borderBottom }"
      );
      assert.equal(
        style.border,
        "none",
...
describe("<HeaderRange />", () => {
  it("consists of proper elements and sets its content correctly", () => {
    ...
    assert(
      wrapper
        ...
        .first()
        .is(Table.HeaderCell),
      "top node contains <Table.HeaderCell />"
    );
    assert.equal(
      wrapper.children().getElements().length,
      1,
      "top node contains just one <Table.HeaderCell />"
    );
    assert.equal(
      wrapper
        ...
        .text(),
      "any text",
      "uses `content` prop as it should"
    );
  });
  ...
  it("has proper styling", () => {
    ...
    assert.equal(
      _.keys(style).length,
      1,
      'top node\'s child has prop style: { border: "none" }'
    );
    assert.equal(
      style.border,
      "none",
